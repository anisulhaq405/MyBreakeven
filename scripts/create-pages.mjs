...
for (const [route, [title, description, heading, privatePage = false]] of Object.entries(staticPages)) {
  ...
  const links = route === "blogs" ? `<ul>${[...articleList, ...day1Posts].map(article => `<li><a href="/blogs/${article.slug}/">${escapeHtml(article.title)}</a></li>`).join("")}</ul>` : `<p><a href="/#calculator">Use the free small business break-even calculator</a></p>`;
