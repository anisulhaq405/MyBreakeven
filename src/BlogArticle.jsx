...
import { blogPostMap, isLongformPost, relatedArticlesFor } from "./content/blogs/index.js";

export default function BlogArticle({slug}){
  const a=blogPostMap[slug];
  if (isLongformPost(a)) return <LongformArticle article={a} />;
  useEffect(()=>{if(a){
  ...
  return <article className="article-page">
    ...
    <section className="article-summary"><h2>What this number really tells you</h2><p>{a.opening}</p><p>Break-even is reached when contribution from completed {a.unit} covers monthly overhead and any owner-pay or profit target entered. Use actual per-{a.singular} costs and realistic productive capacity, then <a href={`/calculators/${a.calculatorSlug}/`}>calculate your {a.tag.toLowerCase()} break-even point</a> with your own figures.</p></section>
