...
export default function BlogSection(){return <section className="blog-preview" id="blog">
  ...
  <div className="blog-grid">{[...articleList, ...day1Posts].map(a=><article key={a.slug} className={!a.image?"text-card":""}>
    {a.image&&<img src={a.image} alt={a.alt} width="1200" height="675" loading="lazy" decoding="async"/>}
    <div><span>{a.tag}</span><time dateTime={a.published}>Published Sep 10, 2026</time><h3>{a.title}</h3><p>{a.description}</p><a href={`/blogs/${a.slug}/`}>Read the {a.tag} break-even guide →</a></div>
  </article>)}</div>
</section>}
